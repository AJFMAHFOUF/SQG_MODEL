...
lon1,lat1,vor1,u1,v1,psi1,phi1,t1=np.loadtxt('../data_out/SQG_T'+trunc+'_lev_'+level+'_step_'+nstep1+'_expid_'+expid+'.dat',unpack=True)

lon1,lat1,vor0,u0,v0,psi0,phi0,t0=np.loadtxt('../data_out/SQG_T'+trunc+'_lev_'+level+'_step_'+nstep0+'_expid_'+expid+'.dat',unpack=True)
...
if var == 'phi':
	...
	cs = ax.contour(lon2,lat2,field/9.81,linewidths=1.1,transform=ccrs.PlateCarree(),colors='black')
	...
	cs = ax.contour(lon2,lat2,field,linewidths=1.1,transform=ccrs.PlateCarree(),colors='black')
...
cbar.ax.tick_params()
#level='650'
plt.title('SQG '+var+' diff '+level+' hpa '+nstep1+'h - T'+trunc+' expid='+expid)
plt.savefig('../plots/SQG_'+var+'_'+level+'_T'+trunc+'_step_'+nstep1+'_expid_'+expid+'_diff.pdf')
plt.show()
